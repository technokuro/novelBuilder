...
const ACCOUNT_MAIL_CHECK = {
    NO_CHECK: { //アカウントは登録後即時 activate される
        activateKeyType: false,
        activateKey: () => null,
        afterActivateRedirect: '/',
    },
    MAIL_URL: { //アカウント登録後送られるメールにアクティベート用URLが送られる
    ...
    MAIL_NUMBER: { //アカウント登録後送られるメールにアクティベート用番号が送られる
...
const FILE_UPLOAD_CONFIG_LIST = {
    ...
    production: {
        ...
        movie: {
            ...
            extensions: ['mov', 'mp4', 'mpg', 'flv', 'f4v', 'ts',
                '3g2', 'asf', 'rm', 'amc', 'm4v', 'mts', 'wmv', 'mpeg',
                'avi', '3gp', 'mod'],
            localStorage: false,
...
const IS_RUN_TABLE_CREATION = process.argv.find(arg => arg === 'createTables');
const IS_RUN_TABLE_ALTER = process.argv.find(arg => arg === 'alterTables');
const IS_RUN_TABLE_INSERT_MASTER = process.argv.find(arg => arg === 'insertMasterData');
...
module.exports = {
    ...
    LOG_CONFIG: process.env.LOG_CONFIG ? JSON.parse(process.env.LOG_CONFIG) : LOG_CONFIG_LIST[ENV],
    MAIL_CONFIG: process.env.MAIL_CONFIG ? JSON.parse(process.env.MAIL_CONFIG) : configSecret.MAIL_CONFIG_LIST[ENV],
    ADMIN_MAIL_ADDRESS: process.env.ADMIN_MAIL_ADDRESS ? JSON.parse(process.env.ADMIN_MAIL_ADDRESS) : configSecret.ADMIN_MAIL_ADDRESS_LIST[ENV],
    PASSWORD_RESET_LIMIT_MINUTES,
    DB_SETTING: process.env.DB_SETTING ? JSON.parse(process.env.DB_SETTING) : configSecret.DB_SETTING_LIST[ENV],
    DB_INIT_FILES,
    ACCOUNT_REGISTER_RULE: ACCOUNT_REGISTER_RULE_LIST[ENV],
    GOOGLE_OAUTH_SETTING: process.env.GOOGLE_OAUTH_SETTING ? JSON.parse(process.env.GOOGLE_OAUTH_SETTING) : {
        client_id: configSecret.GOOGLE_OAUTH_SETTING_LIST[ENV].client_id,
        client_secret: configSecret.GOOGLE_OAUTH_SETTING_LIST[ENV].client_secret,
    ...
    LINE_OAUTH_SETTING: process.env.LINE_OAUTH_SETTING_LIST ? JSON.parse(process.env.LINE_OAUTH_SETTING_LIST) : {
        ...
        redirect_result: configSecret.LINE_OAUTH_SETTING_LIST[ENV].redirect_result,
        channelAccessToken: configSecret.LINE_OAUTH_SETTING_LIST[ENV].channelAccessToken,
    },
    ...
    SERVER_PORT,
    FILE_UPLOAD_CONFIG: process.env.FILE_UPLOAD_CONFIG ? JSON.parse(process.env.FILE_UPLOAD_CONFIG) : FILE_UPLOAD_CONFIG_LIST[ENV],
    IS_RUN_TABLE_CLEARE,
